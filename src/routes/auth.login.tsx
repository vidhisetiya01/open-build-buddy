...
export const Route = createFileRoute("/auth/login")({
  head: () => ({ meta: [{ title: "Log in — Moneywise" }] }),
  component: Login,
});
  ...
  return (
    <div className="mx-auto min-h-screen max-w-md bg-background px-6 pb-10 pt-6">
      ...
      <div className="mt-8">
        ...
        <p className="mt-2 text-sm text-muted-foreground">Log in to continue managing your money.</p>
      </div>
      ...
      >
        ...
        <div className="space-y-1.5">
          ...
          <Input id="password" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="h-12 rounded-xl" required />
        </div>
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <button type="button" className="font-medium text-primary hover:underline">Forgot?</button>
        </div>
        ...
        <Button type="button" variant="outline" className="h-12 w-full rounded-xl text-base font-medium" onClick={() => navigate({ to: "/dashboard" })}>
